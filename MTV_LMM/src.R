...
Create_hsq_data_Fixed_effects <- function(path_hsq_files, hsq_txt, num_otus, 
                                          Refactor_flag, data=NULL, individuals = All_individuals, 
                                          Fixed_effect_flag = 1, 
                                          config = c("1_GRM_FF", "1_GRM", "2_GRM_FF", "2_GRM")){
  
  # path_hsq_files = path_hsq_files = paste0(dir_path,"Data_files/", TE_path);
  ...
  for(j in 1:num_otus){
    ...
    hsq_time[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "V(G1)/Vp"),2])
    se_hsq_time[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "V(G1)/Vp"),3])
    
    hsq_ind[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "V(G2)/Vp"),2])
    se_hsq_ind[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "V(G2)/Vp"),3])
    
    fixed_effect[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "Fix_eff")+2,1])
    intercept[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "Fix_eff")+1,1])
    
    p_val_hsq[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "Pval"),2])
    logL0[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "logL0"),2])
    logL[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "logL"),2])
  }
  
  if(length(intercept) == 0){
    intercept = rep(NA, length(hsq_time))
    fixed_effect = rep(NA, length(hsq_time))
  }
  hsq_data = data.frame( hsq_time, se_hsq_time, hsq_ind, se_hsq_ind ,p_val_hsq, intercept, fixed_effect, logL0, logL, c(1:num_otus), index)
...
Calculate_TE_func <- function(path_hsq_files,
                             ...
                             Fixed_effect_flag = 0, var_comp_num, ind_1, config){
  ...
  return(hsq_data_NEW_Fixed_effects)
...
Prediction_function <- function(X_train_data, Data, x_train_rel,
                                index, OTU, X_start, T_start, All_individuals,
                                path_hsq, path_fixed_effect = NULL, norm_flag = 0, Fixed_effect_flag = 1,
                                fixed_effect_file = "prev_t_1_times__", plot_flag = 0, config = "2_GRM_FF", meta_data){
  ...
  for(i in 1:length(ids)){
    ...
    meta_data_tmp = tmp[-1,]
    meta_data_new = rbind(meta_data_new, meta_data_tmp)
    
  }
  
  
  path_hsq_files = paste(path_hsq ,OTU, sep = "")
  ...
  if(!is.na(Fixed_effect_flag)){
    ...
    hsq_data_NEW_Fixed_effects = Parse_hsq_data_Fixed_effects_pred(path_hsq_files = path_hsq_files,
                                                                   hsq_txt = "GRM_OTUs_reml__", 
                                                                   num_otus = N, config = config, Fixed_effect_flag  = Fixed_effect_flag,
                                                                   individuals = 1)
  ...
  for(t in T_start:(dim(X_train_data)[1] - 1)){
    ...
    par_test_mat = try(as.matrix(read.table(paste(hsq_txt ,par_index[(t-T_start+1)],".indi.blp", sep = ""), 
                                            sep = "\t", fill = T, header = F))[,1:6], silent=TRUE)
    
    if ('try-error' %in% class(par_test_mat))
    {
      Pred[t+1] = Pred[t]
      next
    }
    ...
    if(Fixed_effect_flag == 1){
      ...
      Pred[t+1] = fixed_intercept[t+1] + fixed_coeff[t+1]*fixed_effect_vec[t] +  g_new[t+1]
      
    }
    
    if(Fixed_effect_flag == 0){
      
      const = mean(y_train_vec[c((t-3):t)])
      g_new[t+1] = g_train[t]
      Pred[t+1] = max(g_new[t+1] + const, 0)
  ...
  TE_Results = hsq_data_NEW_Fixed_effects
  colnames(TE_Results) = c("Time_explainability", "SD_Time_explainability", "p_value", 
                           "intercept", "Fixed_effect",
                           "logL0", "logL", "num_otus", "taxa_index")
  ...
  TE_Results = TE_Results[,c("Time_explainability", "SD_Time_explainability", 
                             "logL0", "logL","taxa_index", "p_value_adjusted")]
  
  TE_Results_taxa =  tail(TE_Results,1)
  TE_Results_taxa[,"taxa_index"] = OTU
  
  Result = list(TE_Results_taxa = TE_Results_taxa, meta_data = meta_data_new, y = y , y_hat = y_hat)
...
Parse_hsq_data_Fixed_effects_pred <- function(path_hsq_files, hsq_txt, num_otus, 
                                              Refactor_flag, data=NULL, individuals = All_individuals, 
                                              Fixed_effect_flag = 1, 
                                              config = c("1_GRM_FF", "1_GRM", "2_GRM_FF", "2_GRM")){
  ...
  for(j in 1:num_otus){
    ...
    hsq_time[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "V(G)/Vp"),2])
    se_hsq_time[j] = as.numeric(OTU_data[[j]][which(OTU_data[[j]][,1] == "V(G)/Vp"),3])
