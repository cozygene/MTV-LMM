...
Prediction_function <- function(X_train_data, Data, x_train_rel,
                                ...
                                path_hsq, path_fixed_effect = NULL, norm_flag = 0, Fixed_effect_flag = 0,
  ...
  for(t in T_start:(dim(X_train_data)[1] - 1)){
    ...
    if(!is.na(par_index[(t-T_start+1)])){
      ...
      if(t >= T_start){
        ...
        if(norm_flag == 1){
          ...
          C_data_train = x_train_rel[c(1:t),index]
          # C_data_train = x_train_rel[c(1:t),index]
          ...
          t_W = t(x_train_rel[c(1:t),index])
          # dim(t_W)
          
          u_hat = (t_W %*% Inverse_A)  %*% (g_train)/dim(t_W)[1]
          
          
          # Attr <- attributes(scaled.dat)
          # Attr$`scaled:center`
          # Attr$`scaled:scale`
          # u_hat <- (u_hat- Attr$`scaled:center`)/Attr$`scaled:scale`
      ...
      if(Fixed_effect_flag == 1){
        ...
        if(norm_flag == 1){
          ...
          C_data_test = X_train_data[c(1:t),index]
          # C_data_train = x_train_rel[c(1:(t+1)),index]
          ...
          g_new[t+1] = as.numeric(x_train_rel[t,] %*% u_hat)
        }
        ...
        if(norm_flag == 0){
          ...
          g_new[t+1] = as.numeric(C_data_test_scaled[(t+1),] %*% u_hat)
        }
        ...
        # Pred[t+1] = fixed_effect_vec[t+1]
        Pred[t+2] = fixed_intercept[t+1] + fixed_coeff[t+1]*fixed_effect_vec[t+1] +  g_new[t+1]
        # Pred[t+1] = fixed_intercept[t+1] + fixed_coeff[t+1]*fixed_effect_vec[t+1] +  g_new[t+1] # ORIG
      ...
      if(Fixed_effect_flag == 0){
        ...
        if(norm_flag == 1){
          ...
          g_new[t+1] = as.numeric(as.vector(scaled.dat_test[(t+1),]) %*% u_hat)
        }
        ...
        Pred[t+2] = g_new[t+1] #Only random effects
      }
    ...
    # print(g_new)
    # if(t %% 50 == 0)
    #   print(cor.test(g_train, g_new[c(1:length(g_train))]))
  }
  
  y = c(NA,y_train_vec)
  y_hat = Pred
  Result = list(y = y , y_hat = y_hat)
